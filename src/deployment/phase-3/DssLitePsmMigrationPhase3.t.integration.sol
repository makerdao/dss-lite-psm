...
contract DssLitePsmMigrationPhase3Test is DssTest {
    ...
    function _checkMigrationPhase3() internal {
        ...
        assertEq(dss.vat.Line(), pglobalLine + expectedDstLine - pdstLine - psrcLine, "after: invalid global line");
        ...
        // Old PSM state is cleared correctly.
        {
            ...
            // We cannot guarantee the balance is zero after the migration because of donations.
            assertEq(
                _amtToWad(gem.balanceOf(address(srcPsm.gemJoin()))),
                _amtToWad(psrcGemBalance) - psrcInk,
                "after: invalid gem balance for src"
            );
        }
