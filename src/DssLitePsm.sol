...
interface VatLike {
    ...
    function urns(bytes32 ilk, address u) external view returns (uint256 ink, uint256 art);
}
...
contract DssLitePsm {
    ...
    function fill() public returns (uint256 wad) {
        return _doFill(_subCap(gem.balanceOf(address(keg)) * to18ConversionFactor, cash()));
    }
    ...
    function cut() public view returns (uint256 wad) {
        ...
        return _subCap(dai.balanceOf(address(this)), art + gem.balanceOf(address(keg)) * to18ConversionFactor);
    }
    ...
    function rush() public view returns (uint256 wad) {
        ...
        return _min(
            ...
            _subCap(gem.balanceOf(address(keg)) * to18ConversionFactor, cash()),
    ...
    function gush() public view returns (uint256 wad) {
        ...
        return _min(
            // Cannot burn more than the current debt
            Art,
            _min(
                ...
                _max(
                    ...
                    _subCap(_cash, gem.balanceOf(address(keg)) * to18ConversionFactor),
    ...
    function accDai() public view returns (uint256) {
        return dai.balanceOf(address(this));
    }

    function accGem() public view returns (uint256) {
        return gem.balanceOf(address(keg));
    }
}
