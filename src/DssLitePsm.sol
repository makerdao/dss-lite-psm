...
contract DssLitePsm {
    ...
    /// @dev `wad` precision.
    uint256 public cut;
    /// @notice Tracks this contract Dai balance generated by swaps.
    /// @dev `dai.balanceOf()` is not trusted because Dai transfers directly to this contract would break the accounting.
    uint256 public accDai;
    /// @notice Tracks `keg` `gem` balance generated by swaps.
    /// @dev `gem.balanceOf()` is not trusted because `gem` transfers directly to this contract would break the accounting.
    uint256 public accGem;
    ...
    function sellGem(address usr, uint256 gemAmt) external returns (uint256 daiOutWad) {
        ...
        accDai -= daiOutWad;
        // `accGem <= gem.totalSupply`, so the overflow would have already happened.
        unchecked {
            accGem += gemAmt;
        }
    ...
    function buyGem(address usr, uint256 gemAmt) external returns (uint256 daiInWad) {
        ...
        // `accDai <= dai.totalSupply`, so the overflow would have already happened.
        unchecked {
            accDai += daiInWad;
        }
        accGem -= gemAmt;
    ...
    function fill() public returns (uint256 wad) {
        return _doFill(_subCap(accGem * to18ConversionFactor, cash()));
    }
    ...
    function _doFill(uint256 max) internal returns (uint256 wad) {
        ...
        // `accDai <= dai.totalSupply`, so an overflow is not possible.
        unchecked {
            accDai += wad;
        }
    ...
    function trim() external returns (uint256 wad) {
        ...
        // Underflow not possible, it would underflow first in `daiJoin.join` above.
        unchecked {
            accDai -= wad;
        }
    ...
    function rush() public view returns (uint256 wad) {
        ...
        return _min(
            ...
            _subCap(accGem * to18ConversionFactor, cash()),
