...
library DssLitePsmMigration {
    ...
    function migrate(DssInstance memory dss, DssLitePsmMigrationConfig memory cfg) internal {
        ...
        // 3.1. Grab the collateral from `src.psm` into the executing contract, debt is left intact, to be cleared on phase 3
        dss.vat.grab(src.ilk, src.psm, address(this), address(this), -_int256(src.ink - cfg.rump), 0);
